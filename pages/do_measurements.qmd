...
p <- dat %>%
  filter(group == "county") %>% 
  ggplot(aes(n, county, fill = units)) +
  geom_col(
    col = 1,
  ...
  scale_x_continuous(
    "Number of Dissolved Oxygen Records", expand = expansion(mult = c(0, 0.1))
  ) +
...
p <- dat %>%
  ...
  geom_vline(xintercept = 0) +
  scale_x_continuous("Number of Dissolved Oxygen Observations", expand = expansion(mult = c(0, 0.1))) +
