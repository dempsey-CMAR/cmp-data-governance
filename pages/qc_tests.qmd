...
The distribution of observations was then reviewed to determined which statistics to use to determine outlying conditions. The mean and 3 * standard deviation was used for relatively normally distributed variables [@RN26877], while upper quartiles were used for skewed distributions. 

These thresholds may be re-evaluated in several years, when more data is available.

# QC Tests

Three QARTOD tests and two-developed tests were applied to the CMAR Water Quality data/

QC tests were applied to each deployment using CMAR-developed R package `qaqcmar`, which is available to view and install from [GitHub](https://github.com/dempsey-CMAR/qaqcmar).
