...
To date, dissolved oxygen (mg / L) has only been measured in two counties[^1] in adjacent waterbodies[^2]. A single set of thresholds was calculated for these counties.
...
The dissolved oxygen (mg / L) observations were normally distributed (@fig-do-hist), and so the mean and standard deviation were used to determine $user_{min}$ and $user_{max}$. The statistics and threshold values are shown in Table 2.
...
#| warning: false
#| fig-height: 4
#| fig-cap: Distribution of the spike value of dissolved oxygen (gm / L) observations (binwidth = 0.1 mg/ L). Dotted orange line indicates $spike_{low}$; dotted red line indicates $spike_{high}$.
