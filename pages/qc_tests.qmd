...
- Flag - summary flag for each var
- CMAR interpretation
- Grouped different depending on the test and variable 
- Include decision table here
- Clarify which were CMAR-developed and which were QARTOD?
- Read through for tense
- Examples here?
...
**does this go here or on a separate page**
The historical data was reviewed carefully prior to calculating thresholds. Depending on the number of observations and the spatial and temporal resolution of observations, data was pooled together or separated by county. Other grouping variables (e.g., sensor type) were also used when applicable (**mention depth here??**).
...
QC tests were applied using CMAR-developed R package `qaqcmar`, which is available to view and install from [GitHub](https://github.com/dempsey-CMAR/qaqcmar).
...
- text here
- fixed in processing (human error, not sensor error)
- no flag
